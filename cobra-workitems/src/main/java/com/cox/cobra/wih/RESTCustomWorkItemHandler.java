 ...
 * <li>ContentType - data type in case of sending data - mandatory for POST,PUT
 * </li>
 ...
 * </ul>
 */
public class RESTCustomWorkItemHandler extends AbstractLogOrThrowWorkItemHandler {

	private static final Logger logger = LoggerFactory.getLogger(RESTCustomWorkItemHandler.class);
	...
	public RESTCustomWorkItemHandler(KieSession ksession) {
		this();
	 ...
	 */
	public RESTCustomWorkItemHandler() {
	 ...
	 */
	public RESTCustomWorkItemHandler(String username, String password) {
		this();
	 ...
	 * @param authUrl
	 */
	public RESTCustomWorkItemHandler(String username, String password, String authUrl) {
		this();
		this.username = username;
		this.password = password;
	 ...
	 * Used when no authentication is required
	 */
	public RESTCustomWorkItemHandler(ClassLoader classLoader) {
		logger.debug("REST work item handler will use http client 4.3 api " + HTTP_CLIENT_API_43);
		this.type = AuthenticationType.NONE;
	 ...
	 *            - password to be used for authentication
	 */
	public RESTCustomWorkItemHandler(String username, String password, ClassLoader classLoader) {
		this();
		this.username = username;
		this.password = password;
		this.type = AuthenticationType.BASIC;
	 ...
	 * @param authUrl
	 */
	public RESTCustomWorkItemHandler(String username, String password, String authUrl, ClassLoader classLoader) {
		this();
		this.username = username;
		this.password = password;
		this.type = AuthenticationType.FORM_BASED;
	...
	public void executeWorkItem(WorkItem workItem, WorkItemManager manager) {
		...
		final String urlStr = (String) workItem.getParameter("Url") + "/" + deploymentId + "/" + processInstanceId + "/"
				+ workItemId;
		System.out.println(urlStr);
		String method = (String) workItem.getParameter("Method");
		final String handleExceptionStr = (String) workItem.getParameter("HandleResponseErrors");
		...
		switch (mode) {
		case SYNC:
			System.out.println("in sunc call");
			...
			try {
				...
				if (responseCode >= 200 && responseCode < 300) {
					...
					results.put("StatusMsg", "request to endpoint " + urlStr + " successfully completed "
				...
				try {
					...
					// no idea if this throws something, but we still don't
					// care!
				}
			}
			break;
		case ASYNC:
			...
			final CloseableHttpAsyncClient asyncHttpClient = HttpAsyncClients.custom().setDefaultRequestConfig(config)
					.build();

			asyncHttpClient.start();
			final Future<HttpResponse> future = asyncHttpClient.execute(((RequestBuilder) methodObject).build(), null);
			final boolean handleException2 = handleException;
			...
			new Thread(new Runnable() {
				...
				public void run() {
					try {
						...
						HttpResponse response = future.get(3, TimeUnit.MINUTES);
						StatusLine statusLine = response.getStatusLine();
						int responseCode = statusLine.getStatusCode();
						Map<String, Object> results = new HashMap<String, Object>();
						HttpEntity respEntity = response.getEntity();
						...
						System.out.println("output of webservice call " + responseBody);
						if (responseCode >= 200 && responseCode < 300) {
						 ...
						 * ; if (manager != null) { RuntimeEngine engine =
						 * manager.getRuntimeEngine(ProcessInstanceIdContext.get
						 * (processInstanceId));
						 * 
						 * engine.getKieSession().getWorkItemManager().
						 * completeWorkItem(workItem2.getId(), results);
						 * 
						 * manager.disposeRuntimeEngine(engine); } else { // in
						 * case there is no RuntimeManager available use
						 * available ksession, // as it might be used without
						 * runtime manager at all
						 * ksession.getWorkItemManager().completeWorkItem(
						 * workItem2.getId(), results); }
						 */
					} catch (Exception e) {
						handleException(e);
					} finally {
						try {
							...
							// no idea if this throws something, but we still
							// don't care!
						}
					}
				}
	...
	protected void setBody(RequestBuilder builder, Map<String, Object> params, String deploymentId,
		...
		if (params.containsKey("Content")) {
			try {
				...
				builder.setHeader("deploymentId", deploymentId);
				builder.setHeader("processInstanceId", processInstanceId);
				builder.setHeader("workItemId", workItemId);
	...
	protected void setBody(HttpRequestBase theMethod, Map<String, Object> params) {
		if (params.containsKey("Content")) {
			...
			((HttpEntityEnclosingRequestBase) theMethod).setEntity(
					new StringEntity((String) content, ContentType.create((String) params.get("ContentType"))));
		}
	}
	...
	/**
	 * This method does the actual request, including the setup for
	 * authorization.
	 * </p>
	 * It is <b>not</b> responsible for cleaning up after the last request that
	 * it does.
	 * </p>
	 * It <i>is</i> responsible for cleaning up after all previous request, such
	 * as for form-based authentication, that happen.
	 * 
	 * @param httpclient
	 *            The {@link HttpClient} instance
	 * @param requestBuilder
	 *            The {@link RequestBuilder} instance
	 * @param params
	 *            The parameters that may be needed for authentication
	 * @return A {@link HttpResponse} instance from which we can extract the
	 *         content
	 */
	protected HttpResponse doRequestWithAuthorization(HttpClient httpclient, RequestBuilder requestBuilder,
		...
		if (type == AuthenticationType.BASIC) {
			...
			// Create AuthCache instance and add it: so that HttpClient thinks
			// that it has already queried (as per the HTTP spec)
			...
			try {
				...
				// weird, but this is the method that releases resources,
	...
	protected Object configureRequest(String method, String urlStr, Map<String, Object> params) {
		...
		String deploymentId = (String) params.get("deploymentId");
		String processInstanceId = String.valueOf(params.get("processInstanceId"));
		String workItemId = String.valueOf(params.get("workItemId"));
		if (HTTP_CLIENT_API_43) {
			...
			if ("GET".equals(method)) {
				...
				setBody(builder, params, deploymentId, processInstanceId, workItemId);
